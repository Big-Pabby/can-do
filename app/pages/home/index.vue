<template>
  <div class="space-y-4">
    <MobileNav />
    <div
      class="border border-[#80D6F7] bg-[#EAF8FE] p-6 rounded-[20px] flex justify-between items-center gap-6"
    >
    ...
    <div class="space-y-2">
      ...
      <div class="grid lg:grid-cols-5 md:grid-cols-3 grid-cols-2 gap-4">
        <div
          v-for="(category, index) in CORE_SERVICE_CATEGORIES"
          :key="index"
        ...
        >
          ...
          <p class="text-[#4B5563] text-xs">
            {{ CORE_SERVICE_CATEGORIES[index].length }} Services
          </p>
          <button
